...
if(isset($_POST["id"]) && !empty($_POST["id"])){
   ...
   if(empty($name_err) && empty($email_err) && empty($homeaddress_err) && empty($fooditem_err) && empty($itemquantity_err) && empty($itemamount_err)&& empty($destinationaddress_err)){
        ...
        $sql = "UPDATE DonorDonations SET name=?, email=?,homeaddress=?,fooditem=?,itemquantity=?, itemamount=?,destinationaddress=? WHERE id=?";
    ...
    if(isset($_GET["id"]) && !empty(trim($_GET["id"]))){
        ...
        $sql = "SELECT * FROM DonorDonations WHERE id = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_num_rows($result) == 1){
                    ...
                    header("location: donorerror.php");
                    exit();
                }
...
<select class="form-control" name = "destinationaddress" >
                            <option option = "" ></option>
                            <option option = "Lodwar" >Lodwar</option>
                            <option option = "Lokichogio" >Lokichgio</option>
                            <option option = "Lokichar" >Lokichar</option>
                            <option option = "Lokori" >Lokori</option>
                            <option option = "Kapedo" >Kapedo</option>
                            <option option = "Kakuma" >Kakuma</option>
                            <option option = "Moyale" >Moyale</option>
                            <option option = "Laisamis" >Laisamis</option>
                            <option option = "Loiyangalani" >Loiyangalani</option>
                            <option option = "ileret" > ileret</option>
                            <option option = "Maralal" >Maralal</option>
                            <option option = "Baragoi" >Baragoi</option>
                            <option option = "Lodosoit" >Lodosoit</option>
                            <option option = "Kauro" >Kauro</option>
                            <option option = "El Dera" >El Dera</option>
                            <option option = "Benane" >Benane</option>
                            <option option = "Longopito" >Longopito</option>
                            <option option = "Barata" >Barata</option>
                            <option option = "Dadaab" >Dadaab</option>
                            <option option = "Wajir" >Wajir</option>
                            <option option = "Buna" >Buna</option>
                            <option option = "Mandera" >Mandera</option>
                            <option option = "Banissa" >Banissa</option>
                        ...
                        <center><a href="donorhome.php" class="btn btn-default">Cancel</a></center>
                    </form>
                </div>
            </div>        
        </div>
